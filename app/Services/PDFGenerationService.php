...
class PDFGenerationService
{
        ...
        try 
        {
            ...
            else 
            {
                ...
                try
                {
                     ...
                     {
                        // do nothing
                        $this->dump_array('data set status :: failed ', []);
                     }
                     else 
                     {
                        // insert into Database.
                        $this->_DBservice->reportDataSet($orderCode, $dataSet);
                        $this->dump_array('data set inserted into DB: ', []);
                     }
                }
                catch(Exception $ex)
                {
                    Log::channel('error')->error('Inser into DB : Dataset: Exception: ' . $ex->getMessage()); 
                    $this->dump_array('$$$-> Exception: ', $ex->getMessage());
                }
            }
    ...
    public function common_getPDFReport_service($orderCode, $show_log = 0)
    {
        ...
        try 
        {
                ...
                // 3. Upload to "Stratus"
                $response_StratusUpdated =  $this->postPDF_to_Stratus($pdf->output(), $orderCode);
                if ($show_log == 1) {
